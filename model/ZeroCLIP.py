...
import sys
from tokenizations import tokenization_bert_word_level as tokenization_bert_segment
...
class CLIPTextGenerator:
    def __init__(self,
        ...
        elif lm_model == 'gpt-2-chn':
            self.lm_tokenizer = tokenization_bert.BertTokenizer(vocab_file=tokenizer_path)
            self.lm_model = GPT2LMHeadModel.from_pretrained(model_path,config=model_config)
            self.lm_tokenizer.bos_token = self.lm_tokenizer.cls_token
            self.lm_tokenizer.eos_token = self.lm_tokenizer.sep_token
            self.context_prefix = self.lm_tokenizer.bos_token

        self.lm_model.to(self.device)
        self.lm_model.eval()

        self.forbidden_tokens = None#np.load(forbidden_tokens_file_path)
    ...
    def get_next_probs(self, i, context_tokens):
        ...
        if self.reset_context_delta and context_tokens.size(1) > 1:
            ...
            # context="Image of", 模型预测下文后，会产生内部矩阵 K V.
            # 是为了减少计算量，因为每次都是相同的前缀
